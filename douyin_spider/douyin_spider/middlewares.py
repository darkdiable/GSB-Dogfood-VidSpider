...
class YoutubePlaywrightMiddleware:
    ...
    async def process_request(self, request, spider):
        ...
        def handle_response(response):
            url = response.url
            if ('googlevideo.com/videoplayback' in url or 'manifest' in url.lower() or 'googleapis' in url) and 'blob:' not in url:
                spider.logger.debug(f'Captured YouTube video/playback URL: {url[:100]}...')
                if not self.video_info.get('video_url'):
                    self.video_info['video_url'] = url
        ...
        try:
            ...
            try:
                play_button = await page.query_selector('button[aria-label*="Play"]') or \
                             await page.query_selector('button.ytp-play-button') or \
                             await page.query_selector('video')
                if play_button:
                    await play_button.click(force=True)
                    await asyncio.sleep(5)
            ...
            self.video_info['title'] = title or 'youtube_video'
            self.video_info['extractor'] = 'playwright'
            ...
            response.meta['video_info'] = self.video_info.copy()
            return response
