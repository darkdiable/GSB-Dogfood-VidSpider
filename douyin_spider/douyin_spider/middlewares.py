...
class InstagramPlaywrightMiddleware:
    ...
    async def process_request(self, request, spider):
        ...
        def handle_response(response):
            ...
            if (('.mp4' in url or 'video' in url.lower() or 'cdninstagram' in url or 'fbcdn' in url or 'instagram.com' in url)
                and 'blob:' not in url
                and url not in self.video_urls
                and len(url) > 50):
                spider.logger.debug(f'Captured video URL: {url[:100]}...')
        ...
        try:
            ...
            try:
                accept_button = await page.query_selector('button:has-text("Allow all cookies")') or \
                               await page.query_selector('button:has-text("Accept")') or \
                               await page.query_selector('button:has-text("Allow essential and optional cookies")')
                if accept_button:
            ...
            try:
                play_button = await page.query_selector('button[class*="play"]') or \
                             ...
                             await page.query_selector('video') or \
                             await page.query_selector('svg[aria-label*="Play"]')
                if play_button:
                    await play_button.click(force=True)
                    await asyncio.sleep(5)
            except:
            ...
            try:
                ...
                if video_element:
                    await video_element.evaluate('el => el.play()')
                    await asyncio.sleep(3)
            except:
                pass
