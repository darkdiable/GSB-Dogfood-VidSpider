...
class BilibiliPlaywrightMiddleware:
    ...
    async def process_request(self, request, spider):
        ...
        def handle_response(response):
            url = response.url
            if (('.m4s' in url or '.mp4' in url or '/video/' in url or '/playurl/' in url) 
                and 'blob:' not in url 
                and url not in self.video_urls
                and url not in self.audio_urls
                and len(url) > 50
                and '.jpg' not in url
                and '.jpeg' not in url
                and '.png' not in url):
                if 'audio' in url.lower() or 'audio' in response.headers.get('content-type', '').lower():
        ...
        try:
            ...
            try:
                play_button = await page.query_selector('button[class*="play"]') or \
                             ...
                             await page.query_selector('.bpx-player-ctrl-play')
                if play_button:
                    await play_button.click(force=True)
                    await asyncio.sleep(3)
            except:
